...
%run each subject
	...
	progress(nSubject,'label','simulating each subject');
	for kS=1:nSubject
		doDebug	= DEBUG && kS==1;
	...
	%evaluate the classifier accuracies
		[h,p,ci,stats]	= ttest(acc,0.5,'tail','right');
		
		if DEBUG
			disp(sprintf('mean accuracy: %.2f%%',100*mean(acc)));
			disp(sprintf('group-level: t(%d)=%.3f, p=%.3f',stats.df,stats.tstat,p));
		end
