...
%
% Notes:
%	This script can perform two actions, either separately or together
%	(depending on the environment and invocation options):
%
%		(i)  Generate one or more ThresholdWeave datasets, optionally
%		caching the results in ".mat" files in the "data_store"
%		directory for later use by step (ii).  Generation of a given
%		dataset is suppressed if an equivalent dataset already exists
%		in the cache; however, if 'forcegen' is true, the cache is
%		ignored and the dataset is generated unconditionally.  When
%		'forcegen' is false, the 'nogen' option suppresses step (i)
%		even if no datasets are cached.
%
%		(ii) Create plots of ThresholdWeave datasets, optionally
%		saving the plots in a ".fig" file in the "figfiles" directory.
%		If freshly generated datasets from step (i) are available, the
%		plots are based on those; otherwise, the plots are based on
%		cached datasets.  If a given dataset is available neither from
%		step (i) nor from the cache, the corresponding plots are
%		suppressed.  The 'noplot' option suppresses step (ii)
%		altogether.  Creation of a ".fig" file depends on the
%		'saveplot' option.
%
%	When the script runs in an environment with ShowFigureWindows,
%	'fakedata' and 'nogen' default to true, and 'noplot' defaults to
%	false; without ShowFigureWindows (thus, in batch runs), 'fakedata'
%	and 'nogen' default to false, and 'noplot' defaults to true.
%	These default provisions are aimed at the following use-cases:
%
%		(a) Quick test (with no preexisting cached datasets):  In an
%		interactive environment supporting figure windows, the
%		invocation "s20150718_plot_thresholds('nogen',false)" rapidly
%		generates fake datasets and plots them.  The same invocation
%		may be augmented with arbitrary plotting options and
%		variable-configuration options to explore their effect.
%		(Setting 'forcegen' to true enables testing with new fake data
%		even when cached datasets exist.)
%
%		(b) Batch data generation followed by interactive plotting:
%		The invocation "s20150718_plot_thresholds('max_cores',6)" in a
%		windowless background context generates datasets and caches
%		them in the "data_store" directory.  Then a separate
%		invocation "s20150718_plot_thresholds" (with no arguments) in
%		a foreground windowing context plots the results.  The latter
%		invocation may be repeated with varying plotting options to
%		obtain a variety of plots, as illustrated below under the
%		Example heading.  If a foreground invocation is issued before
%		the background data generation has completed, the script
%		displays partial results:  it plots whichever datasets *have*
%		been generated, and skips the rest.
%
%	The Notes section of the comments in ThresholdWeave discusses how
%	the datasets are generated.  ThresholdWeave supports arbitrary
%	choices for its x-axis and y-axis variables, but in its use here
%	the x-axis variable is hard-coded to be SNR, and the supported
%	y-axis variables are 'nRun', 'nSubject', 'nTBlock', 'nRepBlock',
%	and 'WStrength' (selected by the 'varname' option).  For now, the
%	y-axis domain values are hard-coded as well.  The ThresholdWeave
%	Notes also include a discussion of the computations used in the
%	'multifit' plots produced by this script.
%
%	The dataset caching mechanism relies on a file-labeling scheme to
%	encode the dataset specifications.  For example, if a dataset
%	pertains to 'nSubject' with domain 1:20, and with 0.05:0.002:0.35
%	as the SNR domain, then the dataset's label encodes this
%	information as "nSubject_20_SNR_151_20.0031".  The label also
%	incorporates this script's name in somewhat abbreviated form, to
%	distinguish the dataset from any that may have been generated by
%	other scripts.  However, the labeling scheme is not foolproof.  It
%	is possible for different dataset specifications to yield
%	colliding labels.  As long as this deficiency remains, the user
%	needs to be aware of it and to make evasive maneuvers when
%	collisions threaten.  One such maneuver is to move aside old
%	cached datasets before generating new ones.  (For example,
%	"data_store" files whose names include "s20150718_thresholds"
%	could be relegated to subdirectories of "data_store".)
%
%	A similar labeling issue arises when this script creates ".fig"
%	files.  The names of those files include information about the
%	plotting options in force during figure creation.  Unfortunately,
%	the included information is incomplete, so again collisions are
%	possible, and evasive measures may be needed in some cases.
%
%							*	*	*
%
%	Option-handling in this script is similar to that of
%	s20150615_plot_nSample_wwo_HRF.m, but the latter includes a few
%	niceties that the present script currently omits.  Ideally the
%	applicable code would be factored out and shared across scripts.
%
%	This script is a variant of s20150618_updated_plot_thresholds.m.
%	Among other differences, the variants of this script use different
%	data formats.  Data files written by one variant can be plotted
%	only by the same variant.  Additionally, the present script uses
%	ThresholdWeave, whereas the earlier one used ThresholdSketch.
%
%	See also ThresholdWeave.m, s20150615_plot_nSample_wwo_HRF.m, s20150618_updated_plot_thresholds.m
%
% Example:
%	h = s20150718_plot_thresholds('fakedata',true,'nogen',false,'noplot',false);
%
%	s20150718_plot_thresholds('fakedata',false,'nogen',false,'noplot',true);
%	h = s20150718_plot_thresholds('plottype',{'multifit','test_snr'},'saveplot',true);
%	h = s20150718_plot_thresholds('plotlines',[1 3],'saveplot',true);
%	h = s20150718_plot_thresholds('movavgx',5,'saveplot',true);
%	h = s20150718_plot_thresholds('movavgx',5,'plotlines',[1 3],'saveplot',true);
%	h = s20150718_plot_thresholds('meanyline',5,'saveplot',true);
%	h = s20150718_plot_thresholds('meanyline',5,'plotlines',5,'movavgx',5,'saveplot',true);
%
% Updated: 2015-09-30
...
% stand-alone function, or perhaps into a disk-data memoization class.
	opt		= ParseArgs(varargin, ...
