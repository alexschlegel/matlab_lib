...
% License.
nFillType	= lg.root.info.(lg.type).fill_type;
nInfoCutoff	= lg.root.info.(lg.type).info_cutoff;

nLog	= numel(kLog);

if nLog==1
%just one
	t		= lg.root.info.(lg.type).event.time(kLog);
	strType	= lg.root.info.(lg.type).event.type{kLog};
	ifo		= lg.root.info.(lg.type).event.info{kLog};
...
%multiple
	t		= lg.root.info.(lg.type).event.time(kLog);
	cType	= lg.root.info.(lg.type).event.type(kLog);
	ifo		= lg.root.info.(lg.type).event.info(kLog);
