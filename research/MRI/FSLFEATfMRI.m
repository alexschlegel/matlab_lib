...
%		nthreadmaxfirst:	(5) the maximum number of threads to use for the
%							first level analysis if <firstlevel> is 'subject'
%							(this is because concatenated functional data takes
%							up a lot of memory)
...
%first level analysis
	%should we concatenate?
		if isequal(opt.firstlevel,'subject')
			...
			nThreadFirst	= min(opt.nthreadmaxfirst,opt.nthread);
		else
			nThreadFirst	= opt.nthread;
		end
