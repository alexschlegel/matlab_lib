function [bic,aic] = cca_find_model_order_mtrial(X,Nr,Nl,MINP,MAXP)
%-----------------------------------------------------------------------
...
%           for a multivariate data set; adapted for multitrial data
% 
...
%           Updated April 2009 (multitrial; tested OK)
...
ac = ones(1,MAXP).*999;
for i = MINP:MAXP,
    [Au,Su] = armorf(X,Nr,Nl,i);
    [bc(i),ac(i)] = findbic(Su,nvar,nobs,i);
...
%---------------------------------------------------------------------
function [bc,ac] = findbic(Su,nvar,nobs,nlag)
