...
class Parameters(dict):
	...
	def parse(self):
		...
		self['classifier_param'] = force_list(self['classifier_param'], len(self['classifier']))
		...
		self['partitioner'] = NFoldPartitioner(cvtype=self['leaveout'])
		
		#construct the classifiers
		clf_param = zip(self['classifier'],self['classifier_param'])
		self['classifier'] = [eval(clf)(**p) for clf,p in clf_param]
...
def preprocess_data(param, ds):
	...
	if param['target_balancer']:
		...
		if not param['do_target_balancer'] and param['leaveout'] > 1:
...
def get_partitioner(param, indent=0):
	...
	if param['do_target_balancer']:
		status('using target balancer with %d iterations' % (param['target_balancer_count']), indent=indent+1, debug='all')
		balancer = Balancer(
					attr='targets',
					count=param['target_balancer_count'],
					limit='partitions',
...
def get_cross_validator(param, clf, partitioner, indent=0):
	...
	if param['permutation_test']:
		status('enabling statistics with %d permutations' % (param['permutation_count']), indent=indent, debug='all')
		
		cv_kwargs['postproc'] = mean_sample()
		
		repeater = Repeater(count=param['permutation_count'])
