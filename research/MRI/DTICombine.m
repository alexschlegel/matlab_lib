...
%		nthread:		(1) number of threads to use for processing (only
%						applies when multiple combinations are being performed).
%						note that if the process is being batched, then input of
%						the bet threshold will not be prompted.
...
%   3) combine data for each subject in the data directory, using 5 threads:
%		strDirData  = '~/studies/example/data/';
%		cDirSubject = FindDirectories(strDirData);
%		bSuccess = DTICombine(cDirSubject,'nthread',5);
%	4) search each subject's data directory manually and then combine each set
%	   of data individually.  use 5 threads and don't prompt for a bet
%	   threshold:
%		strDirData  = '~/studies/example/data/';
%		cDirSubject = FindDirectories(strDirData);
