...
classdef Pipeline39e313e  % The suffix 39e313e denotes a 2015-03-18 12:02 git
                          % commit.  This file is the same as Pipeline.m in
                          % that commit, except for (a) the 39e313e suffix on
                          % all occurrences of Pipeline, (b) reporting of
                          % CRecurY in noteVaryingOpts, and (c) this comment.
...
% Simplest invocation (as of 2015-02-03):  Pipeline39e313e.debugSimulation;
% use "Pipeline39e313e.runSimulation" to randomize behavior and suppress
% diagnostic output.  Other parameterless invocations (added later):
%	quickest, no graphics:		Pipeline39e313e.speedupDebugSimulation
%	no graphics, but not quick:	Pipeline39e313e.textOnlyDebugSimulation
%
...
methods
	% Pipeline39e313e - constructor for Pipeline39e313e class
	%
	% Syntax:	pipeline = Pipeline39e313e(<options>)
	%
	% In:
	%	<options>:
	...
	%
	function obj = Pipeline39e313e(varargin)
	...
	function note = noteVaryingOpts(obj,capsule)
		...
		if opt.CRecurX == u.CRecurX && opt.CRecurZ == u.CRecurZ
			cNote{end+1}	= sprintf('CRecurY=%.2f',opt.CRecurY);
		else
			cNote{end+1}	= 'recur=?';
		end
...
methods (Static)
	...
	%   pd=Pipeline39e313e.constructTestPlotData('saveplot',false,'fudge',{'fakecause'},'nSubject',1,'nRun',2)
		pipeline	= Pipeline39e313e(varargin{:});
	...
	%
	% Syntax:	pipeline = Pipeline39e313e.createDebugPipeline(<options>)
	%
	% In:
	%	<options>:
	%		See Pipeline39e313e constructor above for description of <options>,
	...
	%
	% Syntax:	summary = Pipeline39e313e.debugSimulation(<options>)
	%
	% In:
	%	<options>:
	...
	function summary = debugSimulation(varargin)
		pipeline	= Pipeline39e313e.createDebugPipeline(varargin{:});
		summary		= pipeline.simulateAllSubjects;
	end

	% runSimulation - static method for running pipeline
	%
	% Syntax:	summary = Pipeline39e313e.runSimulation(<options>)
	%
	% In:
	%	<options>:
	%		See Pipeline39e313e constructor above for description of <options>
	...
	%                          debug-pipeline
	%
	% Syntax:	summary = Pipeline39e313e.speedupDebugSimulation(<options>)
	%
	% In:
	%	<options>:
	%		See createDebugPipeline above for description of <options>
	...
	%                           debug-pipeline
	%
	% Syntax:	summary = Pipeline39e313e.textOnlyDebugSimulation(<options>)
	%
	% In:
	%	<options>:
	%		See createDebugPipeline above for description of <options>
	...
	%
	% Syntax:	summary = Pipeline39e313e.xWDebugSimulation(<options>)
	%
	% In:
	%	<options>:
	%		See createDebugPipeline above for description of <options>
