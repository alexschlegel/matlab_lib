...
classdef CausalSimulator < handle
	...
	methods
		...
		function data = performAll(obj)
			...
			obj.performPCA(data);
			%data.source.pcaSigs = data.source.funcSigs; %***
			%data.dest.pcaSigs = data.dest.funcSigs;
		...
		function performRegionPCA(~,region)
			...
			region.pcaSigs = zscore(region.pcaSigs); %***
		end
	...
	methods (Static)
		function [data1,data2] = runExample(baseParams)
			if nargin > 0
				bp = baseParams;
			else
				bp = CausalBaseParams;
			...
			%W	= diag(ones(1,bp.numFuncSigs));
			W	= zeros(bp.numFuncSigs); W(1)=1; W(10,10)=1;
			sigGen = SigGenTrigBased(bp,W);
			rng('shuffle');
			voxPol = VoxelPolicy(bp,0.000);
			data1 = CausalSimulator(sigGen,voxPol).runTest;
			%figure; imagesc(data1.wStar); colormap('gray');
			rng('shuffle');
			voxPol = VoxelPolicy(bp,1.000);
			data2 = CausalSimulator(sigGen,voxPol).runTest;
			%figure; imagesc(data2.wStar); colormap('gray');
		end
